...
const Landing_home = () => {
  ...
  const clientID =
    "932914293926-uo3dpst96jr8s51di1mmbhdh3j2gie6a.apps.googleusercontent.com";
  useEffect(() => {
    const start = () => {
      gapi.auth2.init({
        clientId: clientID,
      ...
      <header>
        ...
        <div className="row justtify-content-center align-items-center">
          ...
          {user.email ? (
            <Link
              to="/myaccount"
              className="col-2 col-sm-1 offset-sm-4 col-md-1 offset-md-5 col-lg-1 offset-lg-6"
            >
              🙋‍♂️ {user.firstname}
            </Link>
          ) : (
          ...
          <Link to="/" className="col-2 col-sm-1 col-md-1 col-lg-1">
            🧡 Fav
          </Link>
          <Link to="/" className="col-2 col-sm-1 col-md-1 col-lg-1">
            🛒 Cart
